# 👋 Привет, я Павел Корнев

QA Engineer с коммерческим опытом тестирования веб-приложений и REST API в финтех-домене.  
Основная специализация — ручное тестирование, анализ бизнес-логики и проверка данных.  
Развиваюсь в автоматизации (Java / Python).

---

## 🧠 О себе

- 2 года коммерческого опыта в QA (финтех)
- Тестирование систем взыскания задолженности (сложная бизнес-логика)
- Баланс frontend (~60%) / backend (~40%)
- Опыт работы в Agile (Kanban)
- Участие в 3-amigo сессиях и проработке требований
- Работа в команде: 8 разработчиков + 4 QA

---

## 🛠️ Технологии и инструменты

<p>
  <img src="https://img.shields.io/badge/Postman-FF6C37?logo=postman&logoColor=white"/>
  <img src="https://img.shields.io/badge/Swagger-85EA2D?logo=swagger&logoColor=black"/>
  <img src="https://img.shields.io/badge/MSSQL-CC2927?logo=microsoftsqlserver&logoColor=white"/>
  <img src="https://img.shields.io/badge/Jira-0052CC?logo=jira&logoColor=white"/>
  <img src="https://img.shields.io/badge/Confluence-172B4D?logo=confluence&logoColor=white"/>
  <img src="https://img.shields.io/badge/Kibana-E8478B?logo=elastic&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitLab-FC6D26?logo=gitlab&logoColor=white"/>
  <img src="https://img.shields.io/badge/Jenkins-D24939?logo=jenkins&logoColor=white"/>
  <img src="https://img.shields.io/badge/TeamCity-000000?logo=teamcity&logoColor=white"/>
</p>

---

## 🔍 Основные навыки

### Тестирование
- Functional / Integration / Regression / Smoke testing  
- Анализ требований и выявление несоответствий  
- Проектирование тест-кейсов и чек-листов (TestIT)  
- Ревью тестовой документации  

### API тестирование
- REST API (Postman, Swagger)  
- Проверка бизнес-логики и структуры ответов  
- Валидация данных и статус-кодов  

### Работа с БД
- MSSQL  
- SELECT, JOIN, фильтрация  
- Проверка консистентности данных  

### Диагностика и анализ
- Анализ логов (Kibana, Sentry)  
- Поиск причин дефектов  
- Работа с DevTools  

### CI/CD и процессы
- Jenkins, TeamCity  
- Работа с GitLab (ветки, релизы)  
- Участие в релизах  

---

## 💼 Коммерческий опыт

**Manual QA Engineer — Lime Credit Group**  
Финтех-система сопровождения задолженностей  

### Задачи:
- Анализ и тестирование требований (Confluence)  
- Функциональное и интеграционное тестирование  
- Проверка бизнес-логики расчётов  
- Тестирование REST API  
- Проверка данных в БД  
- Регистрация и сопровождение багов (Jira)  

### Особенности:
- Работа со сложной бизнес-логикой (финансовые расчёты)  
- Участие в 3-amigo сессиях  
- Активное взаимодействие с разработчиками и аналитиками  

---

## 📂 Примеры работ

🔹 Postman коллекции:  
https://github.com/Pablokorne/Postman  

---

## 📊 GitHub статистика

![GitHub stats](https://github-readme-stats.vercel.app/api?username=Pablokorne&show_icons=true)

---

## 💬 Как я работаю

- Начинаю с анализа требований и поиска рисков  
- Проверяю систему на уровне UI, API и БД  
- Особое внимание уделяю бизнес-логике и данным  
- Использую логи для диагностики проблем  
- Стараюсь находить причины дефектов, а не только фиксировать их  

---

## 🚀 В развитии

- Автоматизация тестирования:
  - Java (Selenium / Selenide)
  - Python (pytest, API)
- Улучшение навыков тест-дизайна  
- Изучение архитектуры автотестов  

---

## 📫 Контакты

- Telegram: Kornev_Pablo  
- Email: pashasegak@yandex.ru  

---

## ⚡ Цель

Развиваться в сторону QA Automation Engineer, сохраняя сильную экспертизу в ручном тестировании и глубоком анализе продукта.
